...
#' 
#' Let's load up the AirBnB data.  Remember to *set the working directory* to the folder with the data 
...
#' 
#' 
#' Let's do some basic exploration on the data. How about `select`ing a specific column, and looking 
#' at the first few rows:
## ------------------------------------------------------------------------
head(select(raw_listings, price))

#' 
#' This is fine, but it's a little awkward having to nest our code like that.  Luckily, there is a 
#' nifty operator included with tidyr called the **chaining operator** which looks like `%>%` and 
#' serves like a pipeline from one function to another. Specifically: `x %>% f == f(x)`. Now we can
#'  instead do this:
#' 
## ------------------------------------------------------------------------
...
#' 
#' which is much, much nicer.  Notice that the chaining operator feeds in the object on its left as
#'  the first argument into the function on its right.
#' 
#' You might notice that these prices are actually strings. If we want to work with these as numbers,
#'  we'll need to convert them. Let's try using `mutate`, which allows us to create new columns -- 
#'  often by referring to existing ones.
#' 
## ------------------------------------------------------------------------

#' function() allows us to create a function as an R object
#' replace all occurences of a pattern with gsub
#' we use the blackslash \ to escape special behavior
#' so we replace all dollar signs in the price with space
clean_price <- function(price) as.numeric(gsub('\\$|,', '', price))

#' we use 'mutate' to add new variables to data
raw_listings %>%
...
#' 
#' Maybe we're a little worried these averages are skewed by a few outlier listings. Let's try 'n()'
#' `n()` function here just gives a count of how many rows we have in each group. 
...
#' that we have *outliers* skewing the average.  Because of those outliers, as a rule of thumb, means tend 
#' to be a misleading statistic to use with things like rent prices or incomes.
#' 
...
#' Suppose we want to divide the prices into quartiles, and we want to know the mean rating by both number
#'  of bedrooms and quartile. Furthermore, for each group, we also want to know the mean rating just for listings 
#'  in Back Bay.
#' 
...
#' Hints:
#' 
#' - `cut(x, breaks = quantile(x))` takes a numeric vector `x`, and returns a vector of labels indicating 
#' which quartile each element of `x` falls into.
...
listings %>%
  ...
  summarise(mean_rating = mean(review_scores_rating, na.rm = TRUE),
            mean_back_bay_rating = mean(review_scores_rating[neighbourhood_cleansed
                                                             == 'Back Bay'], na.rm = TRUE))
